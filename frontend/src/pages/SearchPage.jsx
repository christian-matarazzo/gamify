...
export default function SearchPage() {
  ...
  useEffect(() => { fetchSearchResults(urlQuery, urlSort); }, []);
  ...
  const debouncedUpdateUrlAndFetch = (query, sort) => {
    ...
    debounceTimerRef.current = setTimeout(() => updateUrlAndFetch(query, sort), 300);
  };

  const handleQueryChange = (e) => {
    setInputValue(e.target.value);
    debouncedUpdateUrlAndFetch(e.target.value, urlSort);
  };

  const handleSortChange = (e) => updateUrlAndFetch(inputValue, e.target.value);
  ...
  return (
    <main className="container py-4">
      ...
      <div className="d-flex justify-content-between align-items-baseline mb-4">
        <h1 className="gamify-section-title mb-0"><span>Search</span> Games</h1>
        <h1 className="gamify-section-title mb-0"><span>Filter</span> By</h1>
      </div>
      <div className="gamify-search-bar mb-4">
        <div className="gamify-search-input-wrapper">
          ...
          <input type="text" placeholder="Search game..." value={inputValue} onChange={handleQueryChange} className="gamify-search-input" aria-label="Cerca giochi" autoComplete="off" autoCorrect="off" spellCheck="false" />
        </div>
        <select value={urlSort} onChange={handleSortChange} className="gamify-select" aria-label="Ordina risultati">
      ...
      <div className="row g-3">
        {loading ? (
          ...
          <div className="text-center py-5 col-12">
            ...
            <p className="gamify-no-results">No games found{inputValue?.trim() ? ` for "${inputValue.trim()}"` : ''}.</p>
            {inputValue?.trim() && (
              <button className="btn btn-outline-secondary" onClick={() => { setInputValue(''); updateUrlAndFetch('', urlSort); }}>
                Clear search
              </button>
            )}
          </div>
          ...
          games.map(game => {
            ...
            return <GameCard key={game.id} game={game} isInWishlist={isInWishlist} onToggleWish={() => handleWish(game.id)} />;
          })
        )}
      </div>
    </main>
